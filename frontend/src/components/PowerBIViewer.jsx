...
export default function PowerBIViewer({ embedUrl, reportId, isLoading, isListEmpty }) {
  ...
  if (isLoading) {
    return (
      <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center min-h-[600px] w-full h-full text-center p-6">
        <div className="w-12 h-12 border-4 border-gray-100 border-t-secondary rounded-full animate-spin"></div>
        <h3 className="mt-4 text-sm font-semibold text-gray-700">Carregando Dashboards...</h3>
      </div>
    );
  }
  ...
  if (isListEmpty) {
    return (
      <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center min-h-[600px] w-full h-full text-center p-6">
        ...
        <h3 className="text-lg font-bold text-gray-800">Nenhum dashboard disponível</h3>
  ...
  if (!embedUrl) {
    return (
      <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center min-h-[600px] w-full h-full text-center p-6">
        ...
        <h3 className="text-sm font-semibold text-gray-700">Aguardando Seleção...</h3>
  ...
  return (
    ...
    */
    <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-100 min-h-[600px] w-full h-full overflow-hidden flex flex-col relative">
